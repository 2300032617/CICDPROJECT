...
public class CustomerController 
{
   ...
   public ResponseEntity<String> bookEvent(@RequestBody BookEvent bookEvent) {
       try {
           ...
           if (bookEvent.getStartdate() == null || bookEvent.getEnddate() == null) {
               return ResponseEntity.status(400).body("Start date and end date must be provided.");
           }
           if (bookEvent.getBookedcapacity() <= 0) {
               return ResponseEntity.status(400).body("Booked capacity must be greater than 0.");
           }
           ...
           if (customer == null || event == null) {
               return ResponseEntity.status(404).body("Customer or Event not found.");
           }

           // Assign actual objects
